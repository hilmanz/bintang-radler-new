...
class contentHelper {
	...
	function getArticleContent($start=null,$limit=10,$contenttype=0,$topcontent=array(0,3),$articletype=false,$groupby=false,$author=false,$allcontent=false,$datetimes=false,$usingbadetail=true,$usingbranddetail=true,$checkin=true) {
		...
		$typeofsort = "DESC";
		$qCategories = " ";
		$qSort = " anc.id DESC ,anc.posted_date DESC   ";
		if($articletype=='plan'){
			$typeofsort = "ASC";
	...
	function getContentRepository($strId=null,$gallerytype=0,$limit=10){
		...
		foreach($rqData as $key => $val){
			if(is_file("{$CONFIG['LOCAL_PUBLIC_ASSET']}article/{$val['files']}")) {
				// pr($total);
				$rqData[$key]['image_full_path'] = "{$CONFIG['BASE_DOMAIN_PATH']}public_assets/article/{$val['files']}";	
				
				@$qData[$val['otherid']]['total']++;
				// pr($total);
			}else 	{
				$rqData[$key]['image_full_path'] =  $CONFIG['BASE_DOMAIN_PATH']."public_assets/article/default.jpg";	
			}
