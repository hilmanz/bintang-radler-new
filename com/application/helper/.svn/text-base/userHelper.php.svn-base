...
class userHelper {
	...
	function getUserProfileEdit(){
		...
		if($uid!=0 || $uid!=null) {
		$sql = "
			SELECT sm.id,sm.name,sm.last_name,sm.img,sm.sex,sm.username,sm.nickname,sm.register_date,sm.StreetName,sm.phone_number,sm.email,sm.last_login,sm.n_status,sm.sex,sm.birthday,cityref.city as cityname FROM admin_users sm
	...
	function getUserProfile(){
		...
		if($uid!=0 || $uid!=null) {
			...
			$sql = "
				...
				LEFT JOIN admin_users sm ON sm.id=entourage.referrerbybrand 
			...
			if ($_FILES['myImage']['size'] <= 2560000) {
				...
				if($dataImage['result']){
					if ($widget=='photo_profile') {
						...
						/* 	$sql = "UPDATE admin_users SET  img = '{$dataImage['arrImage']['filename']}' WHERE id={$this->uid} LIMIT 1";
							$this->logger->log($sql);
	...
	//global user id, for list of friend of friend : 21,23,1,5,3
		...
		if($uid!=0 || $uid!=null) {
			...
			( 
				...
				( 
					(
					SELECT id,{$uid} userid,ownerid friendid,1 ftype,created_date date_time,n_status 
					FROM my_pages 
					WHERE name  REGEXP '{$groupname}' {$qBrandarr} 
					)
					UNION
					( 
					SELECT id,userid,friendid,ftype,date_time,n_status 
					FROM my_circle 
					WHERE groupid IN ({$strGroupid}) AND userid IN ({$uid}) AND  n_status IN (1) AND ftype=1 GROUP BY friendid,ftype  
					)
					UNION
					( 
					SELECT id,userid,friendid,ftype,date_time,n_status 
					FROM my_circle 
					WHERE groupid IN ({$strGroupid}) AND userid IN ({$uid}) AND  n_status IN (1) AND ftype=0 GROUP BY friendid,ftype  
					)
				) a
	...
	// pr($friends);
			...
			( 
					...
					FROM my_pages mp
					LEFT JOIN admin_users sm ON sm.id=mp.ownerid
					WHERE mp.name REGEXP '{$groupname}'  AND  mp.ownerid <> {$uid} {$qBrandarr} 
					)
					UNION
					( 
					SELECT   mc.id, mc.userid, mc.friendid, mc.ftype, mc.date_time, mc.n_status ,sm.name, sm.last_name
					FROM my_circle mc
					LEFT JOIN admin_users sm ON sm.id=mc.friendid
					WHERE  mc.groupid IN ({$strGroupid}) AND  mc.userid IN ({$uid}) AND  mc.friendid <> {$uid} AND  mc.n_status IN (1) AND  mc.ftype=1 
					GROUP BY  mc.friendid, mc.ftype 
					)
					UNION
					( 
					SELECT mc.id, mc.userid, mc.friendid, mc.ftype, mc.date_time, mc.n_status ,me.name, me.last_name
					FROM my_circle mc
					LEFT JOIN my_entourage me ON me.id=mc.friendid
					WHERE  mc.groupid IN ({$strGroupid}) AND  mc.userid IN ({$uid}) AND  mc.friendid <> {$uid} AND  mc.n_status IN (1) AND  mc.ftype=0 
					GROUP BY  mc.friendid, mc.ftype   
					)
 ...
 */					$result = $this->apps->query($sql);	
	...
	function getSearchFriends(){
		...
		SELECT count(*) total FROM
		(
		( SELECT id,n_status,name,last_name,email FROM admin_users sm WHERE n_status =1  {$qKeywords} ORDER BY name ASC  )
		UNION 
		( SELECT id,n_status,name,last_name,email FROM my_entourage me WHERE n_status =1 AND referrerbybrand = {$this->apps->user->id}   {$qKeywords} ORDER BY name ASC  )
		) a ";
		...
		(
		( SELECT id,n_status,name,last_name,email,img,1 ftype FROM admin_users sm WHERE n_status =1  {$qKeywords}  )
		UNION 
		( SELECT id,n_status,name,last_name,email,img,0 ftype FROM my_entourage me WHERE n_status =1 AND referrerbybrand = {$this->apps->user->id}  {$qKeywords}  )
		) a
		ORDER BY name ASC, last_name ASC LIMIT {$start},{$limit}";
		$qData = $this->apps->fetch($sql,1);
		// pr($qData);
		$this->logger->log($sql);
		if(!$qData) return false;
	...
	function getbaengagement($limit=5,$authorids=false){
		...
		LEFT JOIN admin_users sm ON sm.id=checkin.userid
	...
	function gethirarkidata($type='sba',$leadid=false){
		...
		if($type=='brand') $qSearchUser = " AND pages.type=3 AND pages.ownerid IN ({$brandallid}) "; 
			...
			$sql ="
					...
					LEFT JOIN admin_users sm ON sm.id=pages.ownerid
					WHERE 1 
