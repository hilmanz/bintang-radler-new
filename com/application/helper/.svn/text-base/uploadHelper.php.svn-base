...
class uploadHelper {
	...
	function yoggyImageGenerator(){
		//photo_frame.png 445x114
		global $CONFIG;

		$dest = imagecreatefrompng($CONFIG['LOCAL_PUBLIC_ASSET'].'/yoggy/default.png');
		$share = imagecreatefrompng($CONFIG['LOCAL_PUBLIC_ASSET'].'/yoggy/share.png');
		$filename = uniqid();
		$savePath = $CONFIG['LOCAL_PUBLIC_ASSET'].'yoggy/'.$filename.'.png';
		$savePath_share = $CONFIG['LOCAL_PUBLIC_ASSET'].'yoggy/share_'.$filename.'.png';

		$flavor_list = array();
		$flavor_img = array();
		$flavor_name = strip_tags($this->apps->_p('flavor_name'));

		if(!$flavor_name) return false;
		...
		foreach ($flavor_list as $key => $value) {
			$sql = "SELECT flavor_img
					FROM flavour
					WHERE id = {$value} LIMIT 1";
			$f = $this->apps->fetch($sql);

			$categories = $key+1;

			if($f) $flavor_img[] = array('folder'=>$categories,'img'=>$f['flavor_img']);
			else return false;
		}
		
		foreach ($flavor_img as $key => $value) {
			$src_link = $CONFIG['LOCAL_ASSET'].'content/flavour/'.$value['folder'].'/'.$value['img'];
			...
			switch ($value['folder']) {
				...
				case 2:
					$c_x = 56;$c_y = 139;
					$s_x = 56+42;$s_y = 139+50;
					break;
				case 3:
					$c_x = 83;$c_y = 233;
					$s_x = 83+42;$s_y = 233+50;
					break;
				case 4:
					$c_x = 83;$c_y = 330;
					$s_x = 83+42;$s_y = 330+50;
					break;
			...
			imagecopy($dest, $src, $c_x, $c_y, 0, 0, $width, $height);
			imagecopy($share, $src, $s_x, $s_y, 0, 0, $width, $height);
		}

		$thumb = imagecreatetruecolor(107, 225);
		imagealphablending($thumb, false);
		imagesavealpha($thumb, true);
		
		// Resize
		imagecopyresized($thumb, $dest, 0, 0, 0, 0, 107, 225, 218, 456);
		imagepng($thumb, $savePath);
		imagepng($share, $savePath_share);
		
		imagedestroy($thumb);
		imagedestroy($share);
		
		return array('status'=>true,'img_created'=>array(
										'basic'=>$filename.'.png',
										'share'=>'share_'.$filename.'.png'
									)
					);
