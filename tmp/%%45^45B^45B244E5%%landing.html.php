<?php /* Smarty version 2.6.13, created on 2014-07-07 11:00:42
         compiled from application/web/landing.html */ ?>
<div id="body">
    <div id="page">
        <div id="thecontent">
            <div class="page_section" id="age-gateway">
                <div id="container">
                    <div class="center-title">
                        <img src="<?php echo $this->_tpl_vars['basedomain']; ?>
/assets/images/landing/title.png" />
                    </div>
                    <div id="gateway-form">
                    <form class="inputBirth" method="post" action="">
                        <input type="text" name="dday" onBlur="if(this.value=='')this.value='DD';" onFocus="if(this.value=='DD')this.value='';" value="DD"  onchange="handleChange(this,31);" onKeyPress="return isNumberKey(event)" maxlength="2" />
                        <input type="text" name="dmonth" onBlur="if(this.value=='')this.value='MM';" onFocus="if(this.value=='MM')this.value='';" value="MM" onChange="handleChange(this,12);" onKeyPress="return isNumberKey(event)" maxlength="2"  />
                        <input type="text" name="dyear" onBlur="if(this.value=='')this.value='YY';" onFocus="if(this.value=='YY')this.value='';" value="YY" onChange="handleChange(this);" onKeyPress="return isNumberKey(event)" maxlength="2"  />
                        <input type="hidden" name="age_check" value="1" />
                        <p>BY SUBMITTING THIS FORM, YOU AGREE TO THE PRIVACY AND COOKIE POLICY OF THIS WEBSITE. THIS WEBSITE WILL MAKE USE OF COOKIES.</br>
                            TO LEARN MORE, PLEASE READ OUR 
                            <a href="<?php echo $this->_tpl_vars['basedomain']; ?>
toc" target="_blank">PRIVACY AND COOKIE POLICY</a>
                            <a href="http://www.drink-savvy.com/site_en/" target="_blank">Drink-Savvy</a>
                            <a href="http://www.multibintang.co.id/" target="_blank">Company Information</a></p>
                        <div class="row">
                         <input type="submit" value="ENTER" class="button" />
                        </div>
                    </form>
                    </div><!-- end #gateway-form -->
                    <div class="ice-container">
                        <div class="ice4">
                            <img src="<?php echo $this->_tpl_vars['basedomain']; ?>
/assets/images/landing/ice4.png" />
                        </div><!-- end .ice4 -->
                        <div class="ice3">
                            <img src="<?php echo $this->_tpl_vars['basedomain']; ?>
/assets/images/landing/ice3.png" />
                        </div><!-- end .ice4 -->
                        <div class="ice2">
                            <img src="<?php echo $this->_tpl_vars['basedomain']; ?>
/assets/images/landing/ice2.png" />
                        </div><!-- end .ice4 -->
                        <div class="ice1">
                            <img src="<?php echo $this->_tpl_vars['basedomain']; ?>
/assets/images/landing/ice1.png" />
                        </div><!-- end .ice4 -->
                        <div class="flare">
                            <img src="<?php echo $this->_tpl_vars['basedomain']; ?>
/assets/images/landing/lens_flare.png" />
                        </div><!-- end .flare -->
                    </div><!-- end .ice-container -->
                </div><!-- end #container -->
            </div><!-- end #home -->
      </div>
    </div><!-- end #page -->
</div><!-- end #body -->